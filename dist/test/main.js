...
const manipulate = () => {
    ...
    for (let i = 1; i <= lastdate; i++) {
        ...
        let isToday = new Date(year, month, i).setHours(0, 0, 0, 0) > new Date().setHours(0, 0, 0, 0)
            && new Date(year, month, i).getDay() !== 0
            && new Date(year, month, i).getDay() !== 6
            ? "active"
            : "";
        lit += `<li id="${year}-${month}-${i}" class="${isToday}" onClick="selectDate(${year},${month},${i})">${i}</li>`;
    }
...
// Add event listener for calendar-time-btn buttons to replace calendar-events-top content
function sctBtn(id) {
    var prevBtn;
    var nextBtn;
